...
namespace cAlgo.Robots
{
    ...
    //
    //  Drei unabhÃ¤ngige Strategien in einem Bot:
    //    â€¢ HH/LL  â€“ Tageskerzen-Breakout (Trend),    Fenster 00â€“13 UTC
    //    â€¢ ARB    â€“ Asian-Range-Breakout (Trend),    Fenster 07â€“11 UTC
    //    â€¢ RTO    â€“ Return-to-Open (Mean-Reversion),  Fenster 14â€“18 UTC
    //
    //  Slot-System:  TREND-Slot (HH/LL + ARB teilen sich 1 Trade)
    //                REV-Slot   (RTO, 1 Trade)            â†’ beide parallel.
    //
    //  Einheitliches Management:
    //    â€¢ Risiko: RiskPercent % vom AKTUELLEN Guthaben (Compounding)
    //    â€¢ Break-even: ab BeRThreshold R â†’ Stop +BeLockPips ins Plus (nur Trend)
    //    â€¢ H1-Trailing: ab TrailHour unter letztes H1-Tief/Hoch (nur Trend)
    //    â€¢ TP-Trigger-Trail: TP-Level lÃ¶st engen Trail aus (ALLE Strategien)
    //          Abstand pro Strategie: HhllTpTrailGapPips / ArbTpTrailGapPips / RtoTpTrailGapPips
    //    â€¢ Notfall-TP: TP-Level + EmergencyTpPips broker-seitig (fÃ¤ngt Bot-Ausfall)
    //    â€¢ EOD-Exit: alle Trades um EodHour UTC schlieÃŸen
    //
    //  Alle Zeiten UTC (TimeZone = UTC) â†’ standortunabhÃ¤ngig.
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    [Robot(TimeZone = TimeZones.UTC, AccessRights = AccessRights.None)]
    public class MultiStrategyBot : Robot
    {
        // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        //  PARAMETER â€“ Allgemein
        // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        [Parameter("RTO: Trigger (Ã—ATR)", DefaultValue = 0.7, MinValue = 0.1, MaxValue = 2.0, Step = 0.1, Group = "RTO")]
        ...
        [Parameter("RTO: Stop (Ã—ATR)", DefaultValue = 0.5, MinValue = 0.1, MaxValue = 1.5, Step = 0.1, Group = "RTO")]
        ...
        // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        //  PARAMETER â€“ Management (Trend + gemeinsam)
        // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        private class SymCfg
        {
            ...
            public double SpreadFilterPips;         // max. Spread fÃ¼r Entry
        }
        ...
        private class PosState
        {
            ...
            public double TpPrice;         // Ziel-Level (= Trail-AuslÃ¶ser)
            public bool TpTrailActive;     // TP-Level bereits erreicht?
            public TradeType Dir;
            public double LastStopPrice;   // zuletzt gesetzter Stop (fÃ¼r Exit-Slippage)
        ...
        protected override void OnStart()
        {
            ...
            foreach (var raw in SymbolList.Split(','))
            {
                ...
                if (sym == null) { Print("Symbol {0} nicht verfÃ¼gbar â€“ Ã¼bersprungen.", name); continue; }
        ...
        // PlausibilitÃ¤tsprÃ¼fung der Parameter â€“ verhindert stilles Fehlverhalten
        private bool ValidateParameters()
        {
            ...
            if (ArbEnabled && ArbLonStart < ArbAsiaEnd)
                errs.Add(string.Format("ARB: London-Start ({0}) liegt im Asia-Fenster (Ende {1}) â€“ Range unvollstÃ¤ndig.", ArbLonStart, ArbAsiaEnd));
            ...
            if (EodHour <= latestEntry)
                errs.Add(string.Format("EOD-Stunde ({0}) liegt im/vor letztem Entry-Fenster (bis {1}) â€“ Trades wÃ¼rden sofort geschlossen.", EodHour, latestEntry));

            if (errs.Count > 0)
            {
                Print("PARAMETER-FEHLER â€“ Bot wird NICHT gestartet:");
                foreach (var e in errs) Print("  â€¢ " + e);
                return false;
            }
            return true;
        }
        ...
        protected override void OnBar()
        {
            ...
            // Tageswechsel: VerlustzÃ¤hler zurÃ¼cksetzen
            ...
            if (MaxDrawDownPercent > 0)
            {
                ...
                {
                    Print("Max DrawDown {0:F1}% erreicht â€“ Bot pausiert.", ddPct);
                    _halted = true;
                    return;
                }
            }
            ...
            // Signal-Erkennung je Symbol (1Ã— pro abgeschlossener H1-Bar)
        ...
        private bool TryHHLL(SymCfg cfg, DateTime now)
        {
            ...
            // LONG: heutiges hÃ¶heres Hoch + Ã¼ber Vortagesschluss + H1-Schluss Ã¼ber Vortageshoch
        ...
        private bool TryARB(SymCfg cfg, DateTime now)
        {
            ...
            // Breakout mit H1-Schluss-BestÃ¤tigung
        ...
        // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        //  REV-SLOT (RTO â€“ Return-to-Open, Mean-Reversion)
        // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        private void TryRto(SymCfg cfg, DateTime now)
        {
            ...
            double pip = cfg.PipSize;
            // ATR = Ã˜ Tagesspanne der letzten 20 Tage
            double atrSum = 0;
            for (int i = 1; i <= 20; i++)
            ...
            // Ãœberdehnung nach oben â†’ SHORT (Fade)
            if (upExt >= trigPips)
                OpenTrade(cfg, TradeType.Sell, slPips, tpPips, SLOT_REV, "RTO", now);
            // Ãœberdehnung nach unten â†’ LONG (Fade)
                               ...
                               string slot, string strat, DateTime now)
        {
            ...
            {
                Print("{0} {1}: Spread {2:F1}p > Filter {3:F1}p â€“ kein Trade.",
            ...
            // cTrader rechnet SL/TP in "Broker-Pips" â€“ Umrechnungsfaktor
            ...
            // KEIN hartes TP â€“ nur Notfall-TP (TP-Level + Puffer) broker-seitig.
            double emTpBroker = (EmergencyTpPips > 0) ? (tpPips + EmergencyTpPips) * pipMult : 0;
            double? emTp = (emTpBroker > 0) ? (double?)emTpBroker : null;

            // Referenzpreis VOR AusfÃ¼hrung â†’ Einstiegs-Slippage messbar
            ...
            double entrySlip = (dir == TradeType.Buy)
                ? (pos.EntryPrice - refPrice) / cfg.PipSize   // > 0 = schlechter gefÃ¼llt
        ...
        // PositionsgrÃ¶ÃŸe: RiskPercent % vom AKTUELLEN Guthaben (Compounding)
        private double CalcVolume(SymCfg cfg, double slPips)
        {
            ...
            if (units < cfg.Symbol.VolumeInUnitsMin) return 0;  // Stop zu groÃŸ fÃ¼r 1% â†’ kein Trade
            return units;
        }

        // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
        //  POSITIONS-MANAGEMENT
        //    BE + H1-Trail: nur Trend Â· TP-Trigger-Trail: alle Â· EOD: alle
        // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
        private void ManageOpenPositions(DateTime now)
        {
            foreach (var pos in Positions)
            {
                ...
                if (st.IsTrend)
                {
                    ...
                    // H1-Trailing: Stop unter letztes H1-Tief / Ã¼ber letztes H1-Hoch
                ...
                // â”€â”€ TP-Trigger-Trail (alle Strategien) â”€â”€
        ...
        // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
        //  POSITION GESCHLOSSEN â†’ VerlustzÃ¤hler + State aufrÃ¤umen
        // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
        protected override void OnPositionsClosed(PositionClosedEventArgs args)
        {
            ...
            if (pos.Label != null && pos.Label.StartsWith("MSB_"))
            {
                ...
                // Exit-Slippage = Differenz zuletzt gesetzter Stop â†” tatsÃ¤chliche FÃ¼llung
                // (nur bei Stop-/Trail-Close aussagekrÃ¤ftig); > 0 = nachteilig gefÃ¼llt
                double slipExit = 0;
        ...
        // SchlieÃŸkurs aus der Handels-History (fÃ¼r Exit-Slippage)
        ...
        // Einheitliche, exportierbare Journal-Zeile (Pipe-getrennt â†’ CSV-tauglich)
